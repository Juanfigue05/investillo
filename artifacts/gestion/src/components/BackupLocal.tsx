...
import {
  contarNoSincronizadas,
  exportarRespaldoLocal,
  importarRespaldoLocal,
  reintentarErrores,
} from "@/lib/offline-db";
import { procesarPendientes } from "@/lib/sync-engine";
...
export function BackupLocal({ topbar }: { topbar?: boolean }) {
  ...
  useEffect(() => {
    if (isOpen) contarNoSincronizadas().then(setPendientes);
  }, [isOpen]);
  ...
  const handleGuardarLocal = async () => {
    ...
    try {
      ...
      toast({
        title: "Copia guardada localmente",
        description: `Se descargó ${nombreArchivo}.`,
      });
    } catch (err) {
      toast({
        title: "No se pudo guardar la copia local",
  ...
  const handleArchivoSeleccionado = async (
    ...
    try {
      ...
      if (resultado.ok) {
        ...
        setPendientes(await contarNoSincronizadas());
      } else {
        toast({
  ...
  /** Reactiva operaciones con error y solicita su sincronización. */
  const handleReintentar = async () => {
    setProcesando(true);
    try {
      const cantidad = await reintentarErrores();
      await procesarPendientes();
      setPendientes(await contarNoSincronizadas());
      toast({
        title: "Reintento iniciado",
        description: `${cantidad} operación${cantidad === 1 ? "" : "es"} reactivada${cantidad === 1 ? "" : "s"}.`,
      });
    } catch (err) {
      toast({
        ...
        <div className="absolute right-0 top-full mt-2 w-[min(320px,calc(100vw-2rem))] rounded-2xl border border-border bg-card shadow-2xl overflow-hidden z-50">
          <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {pendientes > 0
                  ? `${pendientes} operación${pendientes === 1 ? "" : "es"} sin sincronizar`
          ...
          <div className="p-3 space-y-2">
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2.5 bg-muted text-foreground rounded-xl font-medium hover:bg-muted/80 transition-all text-sm disabled:opacity-50"
            >
            ...
            {pendientes > 0 && (
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2.5 bg-primary text-primary-foreground rounded-xl font-medium hover:bg-primary/90 transition-all text-sm disabled:opacity-50"
              >
