...
:: Trabaja siempre desde la carpeta del proyecto
...
for /l %%N in (1,1,15) do (
    powershell -NoProfile -Command "try { if ((Invoke-WebRequest -UseBasicParsing -Uri 'http://localhost:%PUERTO%/api/healthz' -TimeoutSec 2).StatusCode -eq 200) { exit 0 } else { exit 1 } } catch { exit 1 }"
    if !errorlevel! equ 0 goto :iniciar_sincronizacion
...
echo Iniciando sincronizacion local con el servidor remoto...
if not exist "logs" mkdir "logs"
start "Investillo sincronizacion" /b cmd /c "pnpm.cmd run sync:all >> logs\sincronizacion.txt 2>&1"
